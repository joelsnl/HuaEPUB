...
- `core/translator.py` - Google / LibreTranslate / Ollama: concurrent translation with bounded multi-pass retry (hard cap on passes; partial improvements are accepted so it never loops forever). Translation cache writes are batched (`put_translation(..., commit=False)` + `flush()`). Re-exports Ollama helpers from `ollama_setup.py`.
...
- `core/polish/` - llama.cpp download/serve, hardware caps (3B/7B/14B), span KEEP/REPLACE. Downloads go through `validate_polish_download_url` (GitHub + Hugging Face https only) and `safe_extract_zip` / `safe_extract_tar`. Verify SHA256 when GitHub publishes `digest` or `HF_GGUF_SHA256` has an official file hash. Never Drive-sync this cache. Do not make polish depend on Ollama.
