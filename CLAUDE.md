...
Modes: **Single**, **Multi** (block-paste URLs), and **Library** (track novels, check/update for new chapters). Optional Google Drive sync for library metadata and/or EPUBs. User data lives under `~/.noveldownloader/` (settings, caches, library, logs, default books folder).
...
- `app.py` - GUI entry point (Single / Multi / Library modes, dark CTk menubar, clipboard watcher)
...
- `core/logger.py` - tee stdout/stderr to `logs/novel_downloader.log` (1 MB startup rotate, keep `.log.1`)
- `core/utils.py` - shared helpers (`safe_filename`, URL extraction, ETA formatting)
- `core/security.py` - URL SSRF guards, safe zip extract, owner-only secret file writes
...
- Drive OAuth uses `drive.file` only; client JSON at `~/.noveldownloader/google_oauth_client.json`. Do not reintroduce hidden `appDataFolder` sync.
- Auto-updates must fail closed without a matching `SHA256SUMS` entry; never fall back to unsigned `main.zip`. Release CI must publish platform zips, `novelDownloader-source.zip`, and `SHA256SUMS.txt`.
